...
function aggregatePreviousWinners(winners) {
    ...
    return Array.from(aggregated.values()).map(entry => {
        const totalPoints = entry.allTimePoints > 0 ? entry.allTimePoints : entry.sumPoints;
        const totalCorrect = entry.allTimeCorrect > 0 ? entry.allTimeCorrect : entry.totalCorrect;
        const totalVotes = entry.allTimeVotes > 0 ? entry.allTimeVotes : entry.totalVotes;
        const bestStreak = entry.allTimeBestStreak > 0 ? entry.allTimeBestStreak : entry.bestStreak;
        const hotSeatAppearances = entry.allTimeHotSeatAppearances > 0
            ? entry.allTimeHotSeatAppearances
            : entry.hotSeatAppearances;
        const hotSeatCorrect = entry.allTimeHotSeatCorrect > 0
            ? entry.allTimeHotSeatCorrect
            : entry.hotSeatCorrect;
